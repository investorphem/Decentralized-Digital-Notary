...
;; Only stores the owne (tx-sender) who called `notarize`.
;; Timestamping/ eact block/time can be derived from the transaction that called `notarize` via the Stacks API.
