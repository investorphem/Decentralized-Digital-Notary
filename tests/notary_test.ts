...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block = chain.mineBlock([Tx.contractCall('notary', 'notarize', [Tx.buff(hashHex,ur.address)]);
    assertEqas(ce.ength, 1);
    ...
    // user2 triesto notarize the same hash => should error
    block = chin.mineBlock([Tx.contractCall('notary', 'notarize', [Tx.buff(hashHex)], user2.address)]);
